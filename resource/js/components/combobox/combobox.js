	...
	ComboBox.prototype={
		...
		*/
		...
		render: function(){
			...
			var oWrap=createElement("div",{className:"ac_wrap",innerHTML:(isIe?'<iframe class="ac_bgIframe"></iframe>':'')+'<div class=ac_wrap_inner><div class=ac_menu_ctn><table cellspacing=0><tbody class=ac_menu></tbody></table></div><table><tbody class=ac_toolbar><tr><td><a href=# class=close>关闭</a></div></td></tr></tbody></table></div>'.replace(/(<\w+)/g,'$1 unselectable="on"')});
			var b=document.body;
			b.insertBefore(oWrap,b.firstChild);
			var els=oWrap.getElementsByTagName("tbody");
			var oText=me.oText, oToolbar=me.oToolbar=els[1], oMenu=me.oMenu=els[0];
			...
			on(me.oText,"keydown",function(e){//监控oText的事件
				...
				switch(kCode){
					...
					case 13 : me.hide();me.fire('enter'); break;//隐藏suggest
				}
