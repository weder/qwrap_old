...
mix(Valid,{
	...
	*/
	...
	checkAll_stamp:1,//checkAll的次数
	isChecking:false,//是否正在checkAll中
	/** 
	* 验证一个表单的所有元素
	* @method checkAll
	* @static
	...
	*/
	checkAll: function (oForm,needFocus,opts){
		...
		if (!isOk && needFocus) window.setTimeout(function(){Valid.check(failEls[0],true,opts);},10);
		return isOk;
	},
	...
	*/
	check:function(el,needFocus,opts){
		if(!Validators.required(el) //非空验证
			|| getAttr2(el,"datatype") && !Validators.datatype(el)
			|| (opts && opts.myValidator && !opts.myValidator(el))//用户自定义验证
		){
			if(needFocus) {
	...
	*/
	initEl:function (el){
		...
		if(dataType=="d" || dataType=="daterange") //Date日期的后面会有日期按钮
		{
			...
			img.onclick=function(e){Utils.getDateFromPopup(el)};
...
var Utils=Valid.Utils={
	...
	*/
	getCalendarImg:(function(){
		...
		return function(){
			return calendarImg=calendarImg || createElement('<img src="'+QW.PATH+'components/valid/assets/Calendar.gif" align="absMiddle" class="calendar-hdl-img" style="cursor:pointer">');
		};
	})(),
	/** 
	* 用日历浮动层来输入一个日期。
	* @method getDateFromPopup
	* @static
	* @param {Element} el 表单元素
	* @return {void}  
	*/
	getDateFromPopup:function(el)
	{
		if (QW.Calendar){
			QW.Calendar.getDateFromPopup(el)
		}
		else
		{
			var calendarJsUrl=QW.PATH+"components/valid/calendar.js?v={version}"; //to get the calendarUrl Url.
			getScript(calendarJsUrl,function(){QW.Calendar.getDateFromPopup(el)})
		}
	},
	/** 
...
mix(Validators,[
	{
			...
			if(!isOk){
				...
				if(precision<1){
					if((/\D/).test(val) || val.length>len)	
						msg=tmpl(Msgs.getMsg(el,"n_integer"),[1+new Array(len+1).join("0")]);
				}
				else {
					var s="^\\d{1,"+(len-precision) +"}(\\.\\d{1,"+precision+"})?$";
		...
		*/
		nrange:function(el,renderResult,pattern){
			...
			if(isOk){
				var fromNEl=$(getAttr2(el,'fromNEl'));
				var toNEl=$(getAttr2(el,'toNEl'));
				if(fromNEl){
					toNEl=el;
				}
				else if(toNEl){
		...
		*/
		daterange:function(el,renderResult){
			...
			if(isOk){
				var fromDateEl=$(getAttr2(el,'fromDateEl'));
				var toDateEl=$(getAttr2(el,'toDateEl'));
				if(fromDateEl){
					toDateEl=el;
				}
				else if(toDateEl){
		...
		*/
		_checkLength:function(el,renderResult,getLengthFun,dataType){
			...
			var msg=null;
			if(!isOk){
				var maxLen=(getAttr2(el,"maxLength")||1024)*1;
				var minLen=(getAttr2(el,"minLength")||0)*1;
				var curLen = getLengthFun(el);
				if(curLen>maxLen) msg=tmpl(Msgs.getMsg(el,"text_longer") || Msgs[dataType+"_longer"],[maxLen,curLen]);
				else if(curLen<minLen) msg=tmpl(Msgs.getMsg(el,"text_shorter") || Msgs[dataType+"_shorter"],[minLen,curLen]);
				else isOk=true;
			}
			if(renderResult!=false) Valid.renderResult(el,isOk,msg)
			return isOk;
		...
		*/
		text:function(el,renderResult){
			return Validators._checkLength(el||this,renderResult,function(a){return getValue(a).length;},"text");
		},
		...
		*/
		bytetext:function(el,renderResult){
			return Validators._checkLength(el||this,renderResult,function(a){return byteLen(getValue(a));},"text");
		},
		...
		*/
		reconfirm:function(el,renderResult){
			Utils.prepare4Vld(el);
			var oriEl=$(el.getAttribute("reconfirmFor"));
		...
		reg:function(el,renderResult,pattern,msg,ignoreDBC){
			...
			if(!isOk){
				...
				if('string' == typeof pattern) {
					pattern.replace(/^\/(.*)\/([ig]*)$/g,function(a,b,c){pattern=new RegExp(b,c||'');});
				}
		...
		*/
		logicrequired:function(el,renderResult,reqlogic){
			...
			var reqAttr=Valid._curReqAttr || Valid.REQATTR,
				...
				sJs=reqlogic.replace(/\$([\w-]+)/ig,'opts.Validators.notempty($("$1"))').replace(/this/ig,"opts.Validators.notempty(opts.el)");//注意：如果是用户输入的dataType，这里有可能会注入。----to be fixed
	...
	},
	{
		...
		* @return {boolean}  
		*/
		time:function(el,renderResult){return Validators.reg(el,renderResult,/^(([0-1]\d)|(2[0-3])):[0-5]\d:[0-5]\d$/,"_time",true);},//时间
		/** 
		* 时间验证
		* @method minute
		* @static
		...
		* @return {boolean}  
		*/
		minute:function(el,renderResult){return Validators.reg(el,renderResult,/^(([0-1]\d)|(2[0-3])):[0-5]\d$/,"_minute",true);},//分钟
		/** 
		* 电子邮件
		* @method email
		* @static
		...
		* @return {boolean}  
		*/
		email:function(el,renderResult){return Validators.reg(el||this,renderResult,/^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/,"_email",true);},  
		/** 
		* 手机号
		* @method mobilecode
		* @static
		...
		* @return {boolean}  
		*/
		mobilecode:function(el,renderResult){return Validators.reg(el||this,renderResult,/^(13|15|18)\d{9}$/,"_mobilecode",true);},
		/** 
		* 含区号电话号码
		* @method phone
		* @static
		...
		* @return {boolean}  
		*/
		phone:function(el,renderResult){return Validators.reg(el||this,renderResult,/^0(10|2\d|[3-9]\d\d)[1-9]\d{6,7}$/,"_phone",true);},//不带分机的电话号
		/** 
		* 电话区号
		* @method phonezone
		* @static
		...
		* @return {boolean}  
		*/
		phonezone:function(el,renderResult){return Validators.reg(el||this,renderResult,/^0(10|2\d|[3-9]\d\d)$/,"_phonezone",true);}, 
		/** 
		* 电话号码
		* @method phonecode
		* @static
		...
		* @return {boolean}  
		*/
		phonecode:function(el,renderResult){return Validators.reg(el||this,renderResult,/^[1-9]\d{6,7}$/,"_phonecode",true);},
		/** 
		* 分机号
		* @method phoneext
		* @static
		...
		* @return {boolean}  
		*/
		phoneext:function(el,renderResult){return Validators.reg(el||this,renderResult,/^\d{1,6}$/,"_phoneext",true);},
		/** 
		* 邮编
		* @method zipcode
		* @static
		...
		* @return {boolean}  
		*/
		zipcode:function(el,renderResult){return Validators.reg(el||this,renderResult,/^\d{6}$/,"_zipcode",true);},
		/** 
		* 邮编
		* @method vcode
		* @static
		...
		* @return {boolean}  
		*/
		vcode:function(el,renderResult){return Validators.reg(el||this,renderResult,/^\w{4}$/,"_vcode",true);}
	}
]);
...
(function(){
	...
	createElement=function(tag,opts){
		var opts=opts||{},el=document.createElement(tag);
		for(var i in opts) el[i]=opts[i];
		return el;
	};
Valid._getHintEl = function (el){
	/*
	* _getHintEl: 得到提示em。查找规则：优先查找emEl属性，再次之查找四个nextSibling以内的em，再次之查找parentNode的四个nextSibling以内的em
	*/
	var hintEl=getAttr2(el,"hintEl");
	return hintEl && $(hintEl);
};
Valid._getEmptyHintEl = function (el){
	/*
	* _getHintEl: 得到提示em。查找规则：优先查找emEl属性，再次之查找四个nextSibling以内的em，再次之查找parentNode的四个nextSibling以内的em
	*/
	var hintEl=getAttr2(el,"emptyHintEl");
	return hintEl && $(hintEl);
};
...
Valid._getErrEmEl=function(okEm,autoCreate){
	...
	*/
	...
	if(errEm){
		if(errEm.tagName=="EM" 
			|| !errEm.tagName && (errEm=errEm.nextSibling) && errEm.tagName=="EM") 
			return errEm;
	}
	if(!autoCreate) return null;
	errEm=createElement('em',{className:'error'});
...
Valid.onhint=function(ce){
	var el=ce.target;
	if(!el || !el.type || 'text,hidden,password,textarea,checkbox,radio,select-one,select-multiple'.indexOf(el.type)==-1) return;//IE下，onfocusin会在div等元素触发 
	var hintEl=Valid._getHintEl(el),
		emptyHintEl=Valid._getEmptyHintEl(el);
	hintEl && replaceClass(hintEl,'dark-hint','hint');
	if(emptyHintEl){
		clearTimeout(el.__placeholderTimer||0);
		addClass(emptyHintEl,'dark-emptyhint');
	}
	if(!Validators.required(el,false) && !getValue(el)) return; //如果存在空提示，则进入焦点时不隐藏提示
	if(!Validators.datatype(el,false)) 
		Validators.datatype(el,true); //只有不通过datatype验证时，才需要在焦点进入时验证
}
Valid.onblur=function(ce){
	...
	var	hintEl=Valid._getHintEl(el),
		emptyHintEl=Valid._getEmptyHintEl(el);
	hintEl && replaceClass(hintEl,'hint','darkhint');
	Validators.datatype(el,true);//离开时只作datatype校验
	if(emptyHintEl){
		clearTimeout(el.__placeholderTimer||0);
		el.__placeholderTimer=setTimeout(function(){//在360浏览器下，autocomplete会先blur再change
			if(getValue(el)) addClass(emptyHintEl,'dark-emptyhint');
			else removeClass(emptyHintEl,'dark-emptyhint');
		},600);
	}
}
...
var emptyhint_idx=10000;
Valid.oninitall=function(ce){
	setTimeout(function(){//稍稍延时一下
		W('input,textarea',ce.target).forEach(function(el){
			var placeholder=el.getAttribute('data-placeholder'),
				emptyHintEl=Valid._getEmptyHintEl(el);
			if(placeholder && !emptyHintEl){
				var emptyhintElId='emptyhint-'+emptyhint_idx++;
				emptyHintEl=createElement('span',{id:emptyhintElId,innerHTML:placeholder,className:'emptyhint'});
				emptyHintEl.onclick=function(){try{el.focus();}catch(ex){}};
				el.parentNode.insertBefore(emptyHintEl,el);
				el.setAttribute('emptyhintel',emptyhintElId);
			}
			if(emptyHintEl){
				if((getValue(el)||'').trim()) {
					addClass(emptyHintEl,'dark-emptyhint');
					...
					removeClass(emptyHintEl,'dark-emptyhint');
				}
			}
		});
	},10);
...
Valid.bindPhoneEls=function(opts){
	...
	for(var i=0;i<ids.length;i++) {
		$$(ids[i]).attr('reqMsg',reqMsgs[i]||defaultReqMsgs[i]).attr('dataType',dataTypes[i]).set('maxLength',maxLengths[i]);
	}
	$(ids[0]).setAttribute('reqlogic','(!$'+ids[1]+' && !$'+ids[2]+') || $'+ids[0]);
	$(ids[1]).setAttribute('reqlogic','(!$'+ids[0]+' && !$'+ids[2]+') || $'+ids[1]);
	if(ids.length==4){
		$(ids[3]).setAttribute('reqlogic','$'+ids[0]+' || $'+ids[1]+'|| $'+ids[2]+'|| $'+ids[3]);
	}
}


QW.DomU.ready(function(){Valid.initAll()});
})();
