(function(){
	var faceScts=[
		{
			id:"pao",
			name:"ÅÝÅÝ",
			path:BB.Editor.editorPath+'tifiles/tiface/pao/',
		...
		},
		{
			id:"youa",
			name:"ÓÐ°¡",
			path:BB.Editor.editorPath+'tifiles/tiface/youa/',
	...
	function td_mouseout(e){
		this.className='';
		$('face-sct-review').style.display='none';
	};
	function td_click(e){
		var imgsrc=this.getAttribute('imgsrc');
		if(imgsrc){
			BB.Editor.EditorCmd._tiFaceExec(imgsrc,this.title);
		}
	};
	function a_click(e){
		BB.EventH.preventDefault(e);
		...
		var els=$('face-sct-main').getElementsByTagName("li");
		for(var i=0;i<els.length;i++){
	...
	{
		var sct=faceScts[i];
		html1.push('<li id="face-sct-hd-'+sct.id+'" '+(i?'':'class="selected"')+'><a href="#">'+sct.name+'</a></li>');
		html2.push('<li id="face-sct-main-'+sct.id+'" '+(i?' style="display:none;"':'')+'><table cellpadding=0 cellspacing=0 border=1>');
	...
	html1.push('</ul>');
	html2.push('</ul></div>');
	$("editor_faces_wraper").innerHTML=html1.join('')+html2.join('');
	var els=$("editor_faces_wraper").getElementsByTagName('td');
	for(var i=0;i<els.length;i++){
		els[i].onmouseover=td_mouseover;
		els[i].onmouseout=td_mouseout;
		els[i].onclick=td_click;
	}
	var els=$("face-sct-hd").getElementsByTagName('a');
	for(var i=0;i<els.length;i++){
