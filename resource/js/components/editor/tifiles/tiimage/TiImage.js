...
function cancelFun()
{
	parent.BB.Editor.EditorCmd._tiImageDialog.hide();
}
...
function okFun()
{
	...
	if(true){  // 添加网上图片
		...
		parent.BB.Editor.EditorCmd._tiImageExec(img_url_value, imgFloat);
	}
...
function initImg(imgUrl,imgFloat){
	...
	try{G("img_url").select();G("img_url").focus();}catch(e){;}
}
