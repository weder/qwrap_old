...
/**
window.SpeedMatchConfig={
	players:[
		{
			title:'JQuery 1.4',
			url:'./frameworks/jquery/speedtest.htm'
		},
		{
			title:'QWrap 1.0',
			url:'./frameworks/qwrap/speedtest.htm'
		}
	],
	cases:['body','div','body div']
};
*/
...
SpeedMatch.init=function(ctn){
	...
	html.push(
		'<table><thead id="thead"><tr><th class="matchcases-title">cases</th><th class="framework">',
		titles.join('</th><th class="framework">'),
		'</th></tr></thead>',
		'<tbody id="tbody"><tr><th class="matchcase">',
		SpeedMatchConfig.cases.join('</th>'+new Array(titles.length+1).join('<td class="empty"></td>','')+'</tr><tr><th class="matchcase">',''),
		'</th>'+new Array(titles.length+1).join('<td class="empty"></td>','')+'</tr></tbody>',
		'<tfoot id="tfoot"><tr><th class="score-title"><strong>final time (less is better)</strong></th>',
		new Array(titles.length+1).join('<td class="score">0</td>',''),
		'</tr></tfoot>',
		'</table>',
		'<h2>Legend</h2>',
		'<table id="legend"><tr><th>the faster</th><th>the slower</th><th>exception thrown or zero elements found</th><th>different returned elements</th></tr>',
		'<tr><td class="good"></td><td class="bad"></td><td class="exception"></td><td class="mismatch"></td></tr>',
		'</table>'
	);
